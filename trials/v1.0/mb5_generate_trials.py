import random
import os
import pandas

seed = 10


def split_and_reverse(lst):
    """Reverse each half of an even-length list, preserving the two-block structure."""
    if len(lst) % 2 != 0:
        raise ValueError("List length must be even to split evenly in half.")

    mid = len(lst) // 2
    first_half = lst[:mid][::-1]
    second_half = lst[mid:][::-1]

    return first_half + second_half


def handle_reverse_ordering(trial_list_element, reverse_order_block, reverse_order_list):
    """Handle within-block reversal and/or full-list reversal."""
    if reverse_order_block == "base_rev":
        if reverse_order_list == "rev":
            return split_and_reverse(trial_list_element)[::-1]
        return split_and_reverse(trial_list_element)

    if reverse_order_list == "rev":
        return trial_list_element[::-1]
    return trial_list_element


def reorder_by_indices(lst, indices):
    """Apply a shared trial-order permutation to any length-12 trial-level list."""
    return [lst[i] for i in indices]


def make_seed_group_order(seed_group_number, cur_seed, trial_num_set):
    """
    Create one within-block shuffled order for a seed group.

    The fribble block and fractal block are shuffled separately, using different
    incrementing seeds. The same seed pair can then be crossed with all 2x2
    combinations of fribble set and test-location order across labs.

    With cur_seed=10:
      seed_group 1 uses fribble_seed=10, fractal_seed=11
      seed_group 2 uses fribble_seed=12, fractal_seed=13
      seed_group 3 uses fribble_seed=14, fractal_seed=15
    """
    fribble_seed = cur_seed + (seed_group_number - 1) * 2
    fractal_seed = fribble_seed + 1

    fribble_indices = list(range(trial_num_set))
    fractal_indices = list(range(trial_num_set, trial_num_set * 2))

    random.Random(fribble_seed).shuffle(fribble_indices)
    random.Random(fractal_seed).shuffle(fractal_indices)

    return fribble_indices + fractal_indices, fribble_seed, fractal_seed


def make_lab_assignments(num_labs):
    """
    Assign labs to crossed fribble-set x familiar-location-order x familiarization-time-set combinations.
    There's 6 possible permutations of the familiarization time sets; we choose 3 that are always mutually exclusive for all fam times (no overlap in position for any pairwise combination).
    Each lab gets 2. There's three unique ways to select pairs of familiarization time sets. So every 3 labs cover two full sets of the 3 familiarization permutations.

    The number of labs must be a multiple of 12 so every seed pair appears once
    with each of the 12 combinations:
      set_1 x loc_order_1 x fam_time_set_1
      set_1 x loc_order_1 x fam_time_set_2
      set_1 x loc_order_1 x fam_time_set_3
      set_2 x loc_order_1 x fam_time_set_1
      ...
      set_1 x loc_order_2 x fam_time_set_1
      ...
      set_2 x loc_order_2 x fam_time_set_1
      ...
      set_2 x loc_order_2 x fam_time_set_3

    Prioritizing: (1) balancing familiarization time, then (2) fribble pairings, then (3) test location ordering
    
    """
    if num_labs % 12 != 0:
        raise ValueError(
            "num_labs must be a multiple of 4 so the 2x2 fribble-set x "
            "location-order combinations can be fully crossed for each seed pair."
        )

    crossing = [
        ("set_1", "loc_order_1","fam_time_set_1"),
        ("set_1", "loc_order_1","fam_time_set_2"),
        ("set_1", "loc_order_1","fam_time_set_3"),
        ("set_2", "loc_order_1", "fam_time_set_1"),
        ("set_2", "loc_order_1", "fam_time_set_2"),
        ("set_2", "loc_order_1", "fam_time_set_3"),
        ("set_1", "loc_order_2","fam_time_set_1"),
        ("set_1", "loc_order_2","fam_time_set_2"),
        ("set_1", "loc_order_2","fam_time_set_3"),
        ("set_2", "loc_order_2", "fam_time_set_1"),
        ("set_2", "loc_order_2", "fam_time_set_2"),
        ("set_2", "loc_order_2", "fam_time_set_3"),
    ]

    assignments = []
    for seed_group_number in range(1, (num_labs // 12) + 1):
        for cur_fribble_set, cur_test_familiar_location, cur_familiarization_time_set_name in crossing:
            assignments.append({
                "seed_group_number": seed_group_number,
                "cur_fribble_set": cur_fribble_set,
                "cur_test_familiar_location": cur_test_familiar_location,
                "cur_familiarization_time_set_name": cur_familiarization_time_set_name
            })

    return assignments


def generate_trials(cur_seed=seed, num_labs=96):
    # define trial parameters
    #familiarization_time_sets = [[5, 10, 15], [10, 15, 5], [15, 5, 10]] #old, when we still had a finite list of fam permutations within a lab
    familiarization_time_sets_dict = {
        'fam_time_set_1': [[5, 10, 15], [10, 15, 5]],
        'fam_time_set_2': [[15, 5, 10], [5, 10, 15]],
        'fam_time_set_3': [[10, 15, 5], [15, 5, 10]],
    }
    #reverse_order_block = ["base_block", "base_rev"] #replace reverse_order_block with this uncommented line if we also want to reverse order within fribble/fractal block
    reverse_order_block = ["base_block"] #decision: no within block reverse ordering
    reverse_order_list = ["base", "rev"]
    familiar_role = [0, 1]
    test_time = 5
    trial_num_set = 6
    trial_num = 12

    fribble_dict = {
        'set_1': [["Tripod", "Diamond"], ["SimsDiamond", "Cylinder"], ["Pyramid", "Bowtie"], ["Pacman", "Arrow"], ["Crinkle", "Pumpkin"], ["Bowl", "Gumdrop"]],
        'set_2': [["Pacman", "Cylinder"], ["Bowtie", "Arrow"], ["Tripod", "Bowl"], ["Crinkle", "Diamond"], ["Pyramid", "Gumdrop"], ["SimsDiamond", "Pumpkin"]]
    }

    fractal_dict = {
        'set_1': [["1A", "1B"], ["2A", "2B"], ["3A", "3B"], ["4A", "4B"], ["5A", "5B"], ["6A", "6B"]]
    }
    fractal_sets = ["set_1"]


    #there's 148 valid orders here - can't do all of them so choosing two "inverted" ones
    #there are 7 unique possibilites for the first 6 occurrences, with the constraint of 3x simple/complex and no more than twice in a row
    #there are only 3 unique orders of 6 occurrences with the additional constraints of (a) not starting with the same complexity level twice, (b) avoiding frontloading (3 of the first 4 are the same) and (c) avoiding true alternation ("suspicious coincidence")
    complexity_order_dict = {
        'order_1': ["simple", "complex", "complex", "simple", "complex", "simple", "complex", "simple", "simple", "complex", "simple", "complex"],
        'order_2': ["complex", "simple", "simple", "complex", "simple", "complex", "simple", "complex", "complex", "simple", "complex", "simple"]
    }
    complexity_orders = ["order_1", "order_2"]

    test_familiar_location_dict = {
        'loc_order_1': ["left", "right", "left", "right", "right", "left", "left", "right", "right", "left", "left", "right"],
        'loc_order_2': ["right", "left", "right", "left", "left", "right", "right", "left", "left", "right", "right", "left"]
    }

    rel_image_path = 'stimuli/images/'
    img_ext = ".png"

    #folder to hold trial lists per lab
    trial_lists_root = os.path.join(os.getcwd(), 'trial_lists')
    os.makedirs(trial_lists_root, exist_ok=True)

    #create folder to hold combined lists per lab (created in parallel, for Experiment Builder)
    combined_trial_lists_root = os.path.join(os.getcwd(), 'combined_trial_lists')
    os.makedirs(combined_trial_lists_root, exist_ok=True)

    lab_assignments = make_lab_assignments(num_labs)
    trial_list_id = 1

    for lab_number, assignment in enumerate(lab_assignments, start=1):
        lab_name = f"lab{lab_number}"
        lab_dir = os.path.join(trial_lists_root, lab_name)
        os.makedirs(lab_dir, exist_ok=True)
        combined_lab_dfs = []

        seed_group_number = assignment["seed_group_number"]
        cur_fribble_set = assignment["cur_fribble_set"]
        cur_test_familiar_location = assignment["cur_test_familiar_location"]
        cur_familiarization_time_set_name = assignment["cur_familiarization_time_set_name"]

        lab_order, fribble_seed, fractal_seed = make_seed_group_order(
            seed_group_number=seed_group_number,
            cur_seed=cur_seed,
            trial_num_set=trial_num_set
        )

        print(
            f"{lab_name}: seed_group={seed_group_number}, "
            f"fribble_set={cur_fribble_set}, "
            f"test_familiar_location={cur_test_familiar_location}, "
            f"familiarization_time_set_name={cur_familiarization_time_set_name}, "
            f"fribble_seed={fribble_seed}, fractal_seed={fractal_seed}, "
            f"lab_order={lab_order}"
        )

        #set the familiarization time sets to use
        familiarization_time_sets = familiarization_time_sets_dict[cur_familiarization_time_set_name]
        print(cur_familiarization_time_set_name)
        print(familiarization_time_sets)

        for cur_fractal_set in fractal_sets: #MZ: only keeping here if we ever want to create multiple sets of fractals
            for cur_complexity_order in complexity_orders:
                for cur_familiarization_time_set in familiarization_time_sets:
                    for cur_reverse_order_list in reverse_order_list:
                        for cur_reverse_order_block in reverse_order_block:
                            for cur_familiar_role in familiar_role:
                                print(lab_name, trial_list_id)

                                # handle familiarization_times
                                fam_time_1 = cur_familiarization_time_set[0]
                                fam_time_2 = cur_familiarization_time_set[1]
                                fam_time_3 = cur_familiarization_time_set[2]
                                fam_time_list = [fam_time_2, fam_time_1, fam_time_3, fam_time_3, fam_time_2, fam_time_1,
                                                 fam_time_3, fam_time_1, fam_time_2, fam_time_1, fam_time_3, fam_time_2]

                                # create complexity and test location order lists
                                cur_complexity_list = complexity_order_dict[cur_complexity_order]
                                cur_complexity_list_1 = cur_complexity_list[:trial_num_set]
                                cur_complexity_list_2 = cur_complexity_list[trial_num_set:]
                                cur_test_familiar_location_list = test_familiar_location_dict[cur_test_familiar_location]

                                # handle familiar items
                                cur_familiar_fribble_items = [pair[cur_familiar_role] for pair in fribble_dict[cur_fribble_set]]
                                cur_familiar_fractal_items = [pair[cur_familiar_role] for pair in fractal_dict[cur_fractal_set]]
                                cur_familiar_items = cur_familiar_fribble_items + cur_familiar_fractal_items

                                # handle novel items
                                if cur_familiar_role == 0:
                                    cur_novel_role = 1
                                else:
                                    cur_novel_role = 0

                                cur_novel_fribble_items = [pair[cur_novel_role] for pair in fribble_dict[cur_fribble_set]]
                                cur_novel_fractal_items = [pair[cur_novel_role] for pair in fractal_dict[cur_fractal_set]]
                                cur_novel_items = cur_novel_fribble_items + cur_novel_fractal_items

                                # First apply the lab-specific within-block shuffle ONLY to
                                # the trial identity variables. Familiarization time, Complexity order and familiar
                                # test-location order are not shuffled by the lab_order; they
                                # stay in the exact order specified by cur_complexity_order and
                                # cur_test_familiar_location, with only the requested reverse-order
                                # variants applied below.
                                shuffled_cur_familiar_items = reorder_by_indices(cur_familiar_items, lab_order)
                                shuffled_cur_novel_items = reorder_by_indices(cur_novel_items, lab_order)
                                trial_type_list = (["fribble"] * trial_num_set) + (["fractal"] * trial_num_set)
                                shuffled_trial_type_list = reorder_by_indices(trial_type_list, lab_order)

                                ordered_cur_complexity_list = handle_reverse_ordering(cur_complexity_list, cur_reverse_order_block, cur_reverse_order_list)
                                ordered_cur_test_familiar_location_list = handle_reverse_ordering(cur_test_familiar_location_list, cur_reverse_order_block, cur_reverse_order_list)
                                switched_ordered_cur_test_familiar_location_list = [
                                    "right" if x == "left" else "left"
                                    for x in ordered_cur_test_familiar_location_list
                                ]
                                ordered_fam_time_list = handle_reverse_ordering(fam_time_list, cur_reverse_order_block, cur_reverse_order_list)
                                timeout_ordered_fam_time_list = [2 * t for t in ordered_fam_time_list]
                                ordered_cur_familiar_items = handle_reverse_ordering(shuffled_cur_familiar_items, cur_reverse_order_block, cur_reverse_order_list)
                                ordered_cur_novel_items = handle_reverse_ordering(shuffled_cur_novel_items, cur_reverse_order_block, cur_reverse_order_list)
                                ordered_trial_type_list = handle_reverse_ordering(shuffled_trial_type_list, cur_reverse_order_block, cur_reverse_order_list)

                                # Build image filenames AFTER the final ordering, so the image
                                # complexity label matches the non-shuffled complexity schedule
                                # for that trial position.
                                ordered_cur_familiar_images = [
                                    item + "_" + complexity.capitalize()
                                    if trial_type == "fribble"
                                    else "Frac" + item + "_" + complexity.capitalize()
                                    for item, complexity, trial_type in zip(
                                        ordered_cur_familiar_items,
                                        ordered_cur_complexity_list,
                                        ordered_trial_type_list
                                    )
                                ]
                                ordered_cur_novel_images = [
                                    item + "_" + complexity.capitalize()
                                    if trial_type == "fribble"
                                    else "Frac" + item + "_" + complexity.capitalize()
                                    for item, complexity, trial_type in zip(
                                        ordered_cur_novel_items,
                                        ordered_cur_complexity_list,
                                        ordered_trial_type_list
                                    )
                                ]

                                ordered_cur_familiar_image_names = [im + img_ext for im in ordered_cur_familiar_images]
                                ordered_cur_novel_image_names = [im + img_ext for im in ordered_cur_novel_images]

                                ordered_cur_familiar_images_path = [rel_image_path + im for im in ordered_cur_familiar_image_names]
                                ordered_cur_novel_images_path = [rel_image_path + im for im in ordered_cur_novel_image_names]

                                left_image_1 = [
                                    familiar_item if fam_location == "left" else novel_item
                                    for familiar_item, novel_item, fam_location in zip(
                                        ordered_cur_familiar_image_names,
                                        ordered_cur_novel_image_names,
                                        ordered_cur_test_familiar_location_list
                                    )
                                ]

                                left_image_path_1 = [rel_image_path + im for im in left_image_1]

                                right_image_1 = [
                                    familiar_item if fam_location == "right" else novel_item
                                    for familiar_item, novel_item, fam_location in zip(
                                        ordered_cur_familiar_image_names,
                                        ordered_cur_novel_image_names,
                                        ordered_cur_test_familiar_location_list
                                    )
                                ]

                                right_image_path_1 = [rel_image_path + im for im in right_image_1]

                                left_image_2 = [
                                    familiar_item if fam_location == "left" else novel_item
                                    for familiar_item, novel_item, fam_location in zip(
                                        ordered_cur_familiar_image_names,
                                        ordered_cur_novel_image_names,
                                        switched_ordered_cur_test_familiar_location_list
                                    )
                                ]

                                left_image_path_2 = [rel_image_path + im for im in left_image_2]


                                right_image_2 = [
                                    familiar_item if fam_location == "right" else novel_item
                                    for familiar_item, novel_item, fam_location in zip(
                                        ordered_cur_familiar_image_names,
                                        ordered_cur_novel_image_names,
                                        switched_ordered_cur_test_familiar_location_list
                                    )
                                ]

                                right_image_path_2 = [rel_image_path + im for im in right_image_2]

                                cur_data = {
                                    "trial_number": range(1, trial_num + 1),
                                    "familiar_stimulus": ordered_cur_familiar_images,
                                    "novel_stimulus": ordered_cur_novel_images,
                                    "familiar_stimulus_item": ordered_cur_familiar_items,
                                    "novel_stimulus_item": ordered_cur_novel_items,
                                    "complexity_condition": ordered_cur_complexity_list,
                                    "familiarization_time": ordered_fam_time_list,
                                    "familiar_stimulus_path": ordered_cur_familiar_images_path,
                                    "novel_stimulus_path": ordered_cur_novel_images_path,
                                    "familiar_location_1": ordered_cur_test_familiar_location_list,
                                    "familiar_location_2": switched_ordered_cur_test_familiar_location_list,
                                    "left_image_path_1": left_image_path_1,
                                    "right_image_path_1": right_image_path_1,
                                    "left_image_path_2": left_image_path_2,
                                    "right_image_path_2": right_image_path_2,
                                    "test_time_1": [test_time] * trial_num,
                                    "test_time_2": [test_time] * trial_num,
                                    "familiarization_time_timeout": timeout_ordered_fam_time_list,
                                    "lab": [lab_name] * trial_num,
                                    "fribble_shuffle_seed": [fribble_seed] * trial_num,
                                    "fractal_shuffle_seed": [fractal_seed] * trial_num,
                                    "seed_group": [seed_group_number] * trial_num,
                                    "fribble_set": [cur_fribble_set] * trial_num,
                                    "test_familiar_location_order": [cur_test_familiar_location] * trial_num,
                                    "familiarization_time_set": [cur_familiarization_time_set_name] * trial_num,
                                    "trial_type": ordered_trial_type_list
                                }
                                cur_df = pandas.DataFrame(cur_data)
                                cur_df.to_csv(
                                    os.path.join(lab_dir, 'mb5_trial_list_' + str(trial_list_id) + '.csv'),
                                    index=False
                                )

                                #copy and add to combined trial list for the current lab
                                combined_cur_df = cur_df.copy()

                                #add columns that only exist in the master list
                                combined_cur_df['familiar_stimulus_image'] = ordered_cur_familiar_image_names
                                combined_cur_df['novel_stimulus_image'] = ordered_cur_novel_image_names
                                combined_cur_df['left_image_1'] = left_image_1
                                combined_cur_df['right_image_1'] = right_image_1
                                combined_cur_df['left_image_2'] = left_image_2
                                combined_cur_df['right_image_2'] = right_image_2
                                combined_cur_df['List'] = trial_list_id
                                combined_lab_dfs.append(combined_cur_df)

                                trial_list_id += 1
        
        #write the combined trial list
        pandas.concat(combined_lab_dfs, ignore_index=True).to_csv(
            os.path.join(combined_trial_lists_root, f'mb5_combined_trial_list_{lab_name}.csv'),
            index=False
            )

    return True


if __name__ == '__main__':
    generate_trials()
