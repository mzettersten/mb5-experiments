...
def make_lab_assignments(num_labs):
    """
    Assign labs to crossed fribble-set x familiar-location-order combinations.

    The number of labs must be a multiple of 4 so every seed pair appears once
    with each of the four combinations:
      set_1 x loc_order_1
      set_1 x loc_order_2
      set_2 x loc_order_1
      set_2 x loc_order_2

    This also guarantees that half of labs use set_1, half use set_2, half use
    loc_order_1, and half use loc_order_2.
    """
    if num_labs % 4 != 0:
        raise ValueError(
    ...
    assignments = []
    for seed_group_number in range(1, (num_labs // 4) + 1):
        for cur_fribble_set, cur_test_familiar_location in crossing:
            assignments.append({
...
def generate_trials(cur_seed=seed, num_labs=12):
                    ...
                    for cur_reverse_order_list in reverse_order_list:
                        for cur_familiarization_time_set in familiarization_time_sets:
                            for cur_familiar_role in familiar_role:
                                ...
                                # the trial identity variables. Complexity order and familiar
