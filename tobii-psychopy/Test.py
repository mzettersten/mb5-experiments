...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ellagaze25\\Desktop\\tobii-psychopy\\Test.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Welcome_Screen_Images = visual.ImageStim(
        win=win,
        name='Welcome_Screen_Images', 
        image='materials/bunnies.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    start_experiment_key = keyboard.Keyboard(deviceName='defaultKeyboard')
    # Run 'Begin Experiment' code from code
    
    trialFile = f"trials/mb5_trial_list_{expInfo['order_number']}.csv"
    print(trialFile)
    ...
    Welcome_Screen = data.Routine(
        name='Welcome_Screen',
        components=[Welcome_Screen_Images, start_experiment_key],
    )
    ...
    while continueRoutine:
        ...
        # *Welcome_Screen_Images* updates
        
        # if Welcome_Screen_Images is starting this frame...
        if Welcome_Screen_Images.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Welcome_Screen_Images.frameNStart = frameN  # exact frame index
            Welcome_Screen_Images.tStart = t  # local t and not account for scr refresh
            Welcome_Screen_Images.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Welcome_Screen_Images, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Welcome_Screen_Images.started')
            # update status
            Welcome_Screen_Images.status = STARTED
            Welcome_Screen_Images.setAutoDraw(True)
        
        # if Welcome_Screen_Images is active this frame...
        if Welcome_Screen_Images.status == STARTED:
            # update params
            pass
        ...
        if start_experiment_key.status == STARTED and not waitOnFlip:
            theseKeys = start_experiment_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisTrialLoop in trialLoop:
        ...
        Laughing_Baby = data.Routine(
            name='Laughing_Baby',
            components=[baby_video, key_resp_continue],
        )
